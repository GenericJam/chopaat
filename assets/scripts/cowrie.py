"""cowrie_*.glb — cowrie shell variants for one batched art-direction round.

Ten variants via three methods:
- Method A (procedural): ovoid from a UV sphere, base flatten, lip
  bulges, boolean slit, subdivision.       -> cowrie_a1..a4
...
- Method C (displacement-textured): method-A body plus a displace
  modifier (clouds / voronoi / wave bands) for surface character.
                                            -> cowrie_c1..c3
...
def finalize(obj, name, out_dir):
    """Rest on z=0, decimate to budget, smooth, export, assert budget."""
...
def method_a(name, out_dir, length, width, height, taper, base_squash,
             slit_w, lip_h, wavy=0.0005, displace=None):
    common.reset_scene()

defmodule Chopaat.Screens.GameScreen do
  ...
  The two features compose: an auto/bot game presents on either board.
  In 2D the bots' throws and moves adopt instantly (no tumble wait) and
  the spectator posture is identical — 2D selection taps (`{:cell2d, _}`
  / `{:pawn2d, _}`) are inert on a bot's turn, exactly like the 3D
  pick messages.
  """
  ...
  # On a bot's turn the screen is a spectator — taps are inert, exactly
  # like the 3D pick messages (beads chopaat-u8x × chopaat-27z).
  def handle_info({:tap, {:cell2d, cell_name}}, socket) do
    %{game: game, move: move, khadu: khadu, handoff: handoff} = socket.assigns

    idle? = is_nil(move) and is_nil(khadu) and is_nil(handoff) and game.phase == :assigning
